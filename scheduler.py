...
async def check_results_and_notify(bot: Bot, match_id: int):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not match_obj or match_obj.status == "FT":
            logger.info(
                f"Match {match_id} already processed or not found. Removing job."
            )
            try:
        ...
        if data and data["status"] == "FINISHED":
            ...
            (
                match_obj.actual_home_score,
                match_obj.actual_guest_score,
                match_obj.status,
            ) = ah, ag, "FT"
            ...
            for bet in bets:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to send notification to user {bet.user_id}: {e}"
                    )
...
async def schedule_match_jobs(bot: Bot, match_obj: Match):
    ...
    logger.info(
        f"Scheduled result checking for match {match_obj.id} starting at {start_poll}"
    )
    ...
    if reminder_time > now:
        ...
        logger.info(
            f"Scheduled hourly reminder for match {match_obj.id} at {reminder_time}"
        )
    else:
...
async def daily_match_reminder(bot: Bot):
    ...
    async with AsyncSessionLocal() as session:
        ...
        logger.info(
            f"{len(matches)} match(es) in window. Scheduling jobs and notifying users."
        )
        lines = []
        for match_obj in matches:
